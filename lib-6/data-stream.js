...
const _require = require("./util/promise-transform-stream"),
...
const _require3 = require("./util/utils"),
...
class DataStream extends PromiseTransformStream {
  ...
  static from(input, options, ...args) {
    ...
    const StreamError = scramjet.errors.StreamError;
    ...
    if (input instanceof Readable || typeof input.readable === "boolean" && typeof input.pipe === "function" && typeof input.on === "function") {
      const out = target || new this(Object.assign({}, options, {
    ...
    if (typeof input === "function") {
      ...
      Promise.resolve(options).then(input).then(source => this.from(source, out)).catch(e => out.raise(new StreamError(e, out, "EXTERNAL", null)));
      return out;
    }
  ...
  unorder(func) {
    ...
    function () {
      var _ref2 = _asyncToGenerator(function* (chunk) {
        ...
        if (slot < 0) slot = yield new Promise(res => waiting.push(res));
        processing[slot] = Promise.resolve(chunk).then(func).then(result => out.whenWrote(result)).then(() => {
    ...
    }()).run().then(() => Promise.all(processing)).then(() => out.end());
    return out;
  }
  /**
  ...
  into(func, into) {
    ...
    if (!into._options.referrer) into.setOptions({
      referrer: this
    });
    this.tap().catch(e => into.raise(e)).pipe(new this.constructor({
      promiseTransform: function () {
        var _promiseTransform = _asyncToGenerator(function* (chunk) {
          try {
            yield func(into, chunk);
          } catch (e) {
            into.raise(e);
          }
        });
  ...
  use(func, ...parameters) {
    ...
    if (typeof func === "function") {
      ...
      if (result instanceof Promise) {
        ...
        result.then(res => this.constructor.from(res).pipe(out)).catch(e => out.raise(e));
        return out;
      } else {
        return result;
      }
  ...
  run() {
    ...
    return _asyncToGenerator(function* () {
      return _this2.on("data", () => 0).whenEnd();
    })();
  }
  /**
    ...
    _asyncToGenerator(function* () {
      for (var _i = 0; _i < transforms.length; _i++) {
        ...
        if (transform instanceof Transform || typeof transform.readable === "boolean" && transform.readable && typeof transform.writable === "boolean" && transform.writable && typeof transform.pipe === "function" && typeof transform.on === "function") {
          current = _this3.from(current.pipe(transform));
        } else {
          current = _this3.from(current).use(transform);
        }
      }

      _this3.from(current).pipe(out);
    })().then().catch(e => {
      return out.raise(e);
    });
  ...
  each(func) {
    return this.tap().map(a => Promise.resolve(func(a)).then(() => a)).resume();
  }
  /**
  ...
  toArray(initial) {
    return this.tap().reduce((arr, item) => (arr.push(item), arr), initial || []);
  }
  /**
