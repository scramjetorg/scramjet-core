...
const inheritedProxy = parent => {
  return new Proxy({}, {
    ...
    set(target, prop, value) {
      if (prop in target) throw new Error(`Option "${prop}" already defined`);
      target[prop] = value;
      return true;
    }
    ...
    const value = new Proxy(this, {
      ...
      get(target, key) {
        if (key in target[_declarations]) {
          ...
          if (target[_chain] && target[_declarations][key].chained && target[_chain].hasOwnProperty(key)) return target[_chain][key];
