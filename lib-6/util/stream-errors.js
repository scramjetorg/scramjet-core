...
const combineStack = (stack, ...errors) => {
  return errors.reduce((stack, trace) => {
...
class StreamError extends Error {
  constructor(cause, stream, code = "GENERAL", chunk = null) {
    ...
    super(cause.message);
    if (cause instanceof StreamError) return cause;
    this.chunk = chunk;
    this.stream = stream;
    ...
    Object.defineProperty(this, "stack", {
      get: function get() {
        return combineStack(stack, "  caused by:", cause.stack, `  --- raised in ${stream.name} constructed ---`, stream.constructed);
      }
    });
