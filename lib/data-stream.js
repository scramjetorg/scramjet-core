 ...
 * @extends module:stream.PassThrough
 */
class DataStream extends PromiseTransformStream {
     ...
     * @param {class} [ClassType=this.constructor] The class to be mapped to.
     * @chainable
     *
    ...
    into(func, into) {
        ...
        const passthrough = new (this.constructor)({
            promiseTransform: async (chunk) => func(into, chunk),
            promiseFlush: () => into.end(),
            referrer: this
        });

        if (!into._options.referrer)
            into.setOptions({referrer: passthrough});
     ...
     *
     * @chainable
     * @param {AsyncGeneratorFunction|GeneratorFunction|UseCallback<this>|String|Readable} func if passed, the function will be called on self to add an option to inspect the stream in place, while not breaking the transform chain. Alternatively this can be a relative path to a scramjet-module. Lastly it can be a Transform stream.
 ...
 * @memberof module:scramjet~
 * @param {Buffer|String|*} chunk the stream chunk
