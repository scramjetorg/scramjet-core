...
class DataStream extends PromiseTransformStream {
     ...
     *
     * @chainable
     * @param {Array|Iterable|GeneratorFunction|AsyncFunction|Readable} str any node.js readable stream, function (`objectMode: true` is advised)
     * @param {StreamOptions|Writable} options
     */
    static from(stream, options, ...args) {
        const target = options instanceof this.constructor && options;
        ...
        if (stream instanceof GeneratorFunction)
            return pipeIfTarget(this.fromIterator(stream(...args), options), target);

        if (Array.isArray(stream))
            return pipeIfTarget(this.fromArray(stream, options), target);

        if (stream[Symbol.iterator] || stream[Symbol.asyncIterator])
            return pipeIfTarget(this.fromIterator(stream[Symbol.iterator] || stream[Symbol.asyncIterator], options), target);
    ...
    static fromIterator(iter, options) {
        return new DataStream(Object.assign({}, options, {
            parallelRead() {
