...
class DataStream extends PromiseTransformStream {
     ...
     *
     * @chainable
     * @param {Function|String|Transform} func if passed, the function will be called on self to add an option to inspect the stream in place, while not breaking the transform chain. Alternatively this can be a relative path to a scramjet-module. Lastly it can be a Transform stream.
    ...
    use(func, ...args) {
        if (typeof func === "function") {
            ...
            if (result instanceof Promise) {
                ...
                result.then(
                    res => DataStream.from(res).pipe(out)
                ).catch(
                    e => out.raise(e)
                );
                return out;
        ...
        } else if (func instanceof Transform || (
     ...
     * @param {AsyncFunction|Function|Transform} transforms an array of transorms
     *
     * @returns {DataStream} a new DataStream instance of the resulting pipeline
     */
    static pipeline(readable, ...transforms) {
        const out = new this();
        let current = DataStream.from(readable);
        ...
        (async () => {
            for (let transform of transforms) {
                if (transform instanceof Transform || (
                    ...
                    current = DataStream.from(current.pipe(transform));
                } else {
                    current = DataStream.from(current).use(transform);
                }
            }
