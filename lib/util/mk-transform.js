...
module.exports = ({filter}) => function mkTransform(newOptions) {
    ...
    if (this._scramjet_options.transforms.length) {
        ...
        this._transform = (chunk, encoding, callback) => {
                ...
                .catch(
                    async (e) => {
                        if (e instanceof Error) {
                            return [await this.raise(new StreamError(e, this, "EXTERNAL", chunk), chunk)];
                        } else {
            ...
            ref.then(
                () => ref !== processing.shift() && this.raise(new StreamError(new Error(`Promise resolved out of sequence in ${this.name}!`), this, "TRANSFORM_OUT_OF_SEQ", chunk), chunk)
            );
