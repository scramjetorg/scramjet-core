 ...
 */
module.exports = ({filter, term}) => function mkTransform(newOptions) {
    this.setOptions(
        {
            transforms: [],
    ...
    if (this._scramjet_options.transforms.length) {
        ...
        this._transform = (chunk, encoding, callback) => {
            ...
            const ref = last = Promise
                ...
                .catch(
                    (e) => {
                        if (e === term || e === filter) return [e];
                        if (e instanceof Error) {
                            return this
                                .raise(new StreamError(e, this, "EXTERNAL", chunk), chunk)
                                .catch(e => e === term ? [e] : Promise.reject(e))
                            ;
                        } else {
                ...
                .then(
                    (args) => {
                        if (args[0] === term || args[1] === term) {
                            termed = true;
                            return Promise.reject(term);
                        }
