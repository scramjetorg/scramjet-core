...
class PromiseTransformStream extends Transform {
    ...
    pushTransform(options) {
        ...
        if (typeof options.parallelTransform === "function") {
            ...
            if (before)
                this._options.transforms.push(options.beforeTransform.bind(this));

            if (after)
                this._options.transforms.push(async (chunk) => {
                    return options.afterTransform.call(this, chunk, await options.parallelTransform.call(this, chunk));
                });
            else
                this._options.transforms.push(options.parallelTransform.bind(this));

        }

        if (options.flushPromise)
            this._options.flushPromise = async () => {
    ...
    mkTransform() {
        ...
        this._flush = (callback) => {
            if (this._options.flushPromise) {
                last.then(this._options.flushPromise).then((data) => callback(null, data));
            } else {
                last.then(() => callback());
            }
        };
