...
const mkRead = require('./mk-read')({ filter, DefaultHighWaterMark, plgctor, storector });
const mkWrite = require('./mk-write')({ filter, DefaultHighWaterMark, plgctor, storector });
...
class PromiseTransformStream extends Transform {
    ...
    constructor(options) {
        ...
        if (newOptions.parallelRead) {
            mkRead.call(this, newOptions);
        } else if (newOptions.parallelWrite) {
            mkWrite.call(this, newOptions);
        } else if (newOptions.transform || !newOptions.parallelTransform) {
            this.tap();
        } else {
            if (newOptions.parallelTransform && mkTransform.call(this, newOptions)) { // returns true if transform can be pushed to referring stream
