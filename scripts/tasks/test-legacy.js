...
module.exports = function testLegacy(src) {
    ...
    return () => DataStream.from(gulp.src(src))
        .map(file => ([path.basename(file.path), require(file.path)]))
        .map(([name, tests]) =>
            Object.keys(tests)
                .map(k => {
                    if (typeof tests[k] === "function") {
                        return {[`${name}/${k}`]: tests[k]};
                    } else {
                        return Object.keys(tests[k]).reduce((out, test) => (
                            out[`${name}/${k}:${test}`] = tests[k][test],
                            out
                        ), {});
                    }
                ...
                .reduce(
                    (acc, tests) => Object.assign(acc, tests),
                    {}
                )
        )
        .into(
