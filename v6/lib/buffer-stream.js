...
class BufferStream extends scramjet.DataStream {
   ...
   * @return {BufferStream} sub-stream
   *
  ...
  shift(bytes, func) {
    ...
    function () {
      var _ref = _asyncToGenerator(function* () {
        _this.removeListener("data", chunkHandler);

        _this.removeListener("end", endHandler);
  ...
  stringify(encoding) {
    return this.pipe(new scramjet.StringStream(encoding || "utf-8", {
      objectMode: true
    }));
  }
  /**
   ...
   *
   * @param {Array|Iterable|AsyncGeneratorFunction|GeneratorFunction|AsyncFunction|Function|Readable} stream argument to be turned into new stream
