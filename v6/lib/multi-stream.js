...
class MultiStream extends EventEmitter {
  ...
  constructor(streams, ...args) {
    ...
    this.streams = [];
    if (Array.isArray(streams)) streams.forEach(str => this.add(str));
  }
  /**
  ...
  map(aFunc, rFunc) {
    return Promise.all(this.streams.map(s => {
      return Promise.resolve(s).then(aFunc);
    })).then(streams => {
      const out = new this.constructor(streams);
      this.on("add", stream => Promise.resolve(stream).then(aFunc).then(out.add.bind(out)));
      if (rFunc) this.on("remove", stream => Promise.resolve(stream).then(rFunc).then(out.remove.bind(out)));
      return out;
    });
  }
  /**
  ...
  each(aFunc, rFunc) {
    return Promise.all(this.streams.map(s => {
      ...
      this.on("add", stream => Promise.resolve(stream).then(aFunc));
      if (rFunc) this.on("remove", stream => Promise.resolve(stream).then(rFunc));
      return this;
    });
  }
  /**
  ...
  filter(func) {
    return Promise.all(this.streams.map(s => Promise.resolve(s).then(func).then(o => o ? s : null))).then(streams => {
      const out = new this.constructor(streams.filter(s => s));
      this.on("add", stream => Promise.resolve(stream).then(func).then(out.add.bind(out)));
