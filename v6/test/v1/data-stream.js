...
module.exports = {
  ...
  test_from: {
    ...
    testAsyncIterable(test) {
      return _asyncToGenerator(function* () {
        if (!Symbol.asyncIterator) return test.done();
  ...
  test_while_until: {
    while(test) {
      test.expect(1);
      getStream().while(data => data.val < 50).toArray().then(data => {
    ...
    until(test) {
      test.expect(1);
      getStream().until(data => data.val >= 50).toArray().then(data => {
  ...
  test_tee: {
    standard(test) {
      ...
      str.tee(stream => {
        ...
        test.done();
      }).on("error", e => (console.log(e), test.ok(false, "Should not throw error: " + e)));
    },
  ...
  test_reduce: {
    ...
    pass_accumulator_test(test) {
      test.expect(1);
      getStream().reduce((acc, int) => acc + int.val, 0).then(acc => {
  ...
  test_map(test) {
    ...
    Promise.all([mapped, unmapped]).then(args => {
      const mapped = args[0];
  ...
  test_filter(test) {
    ...
    Promise.all([filtered, unfiltered]).then(args => {
