...
module.exports = {
  test_construct(test) {
    ...
    test.doesNotThrow(() => {
      new MultiStream("0123456789".split("").map(n => getStream(n)));
  ...
  test_mux(test) {
    const streams = [getStream(10, 10, 1), getStream(20, 10, 1)];
    const mux = new MultiStream(streams).mux();
    test.ok(mux instanceof DataStream, "Returns DataStream instance");
    mux.reduce((acc, item) => (acc.push(item.val), acc), []).then(arr => {
  ...
  test_mux_nowarn(test) {
    test.plan(1);
    const streams = [getStream(10, 10, 1), getStream(20, 10, 1), getStream(30, 10, 1), getStream(40, 10, 1), getStream(50, 10, 1), getStream(60, 10, 1), getStream(70, 10, 1), getStream(80, 10, 1), getStream(90, 10, 1), getStream(100, 10, 1), getStream(110, 10, 1), getStream(120, 10, 1)];
    let noWarnings = 0;
    process.on("warning", ({
      name,
      stack
      ...
      console.error(stack);
      if (name === "MaxListenersExceededWarning") noWarnings++;
    });
    const mux = new MultiStream(streams).mux();
    return mux.run().then(() => {
  ...
  test_mux_cmp(test) {
    test.plan(5);
    const streams = [getStream(10, 10, 3), // [10, 13, 16, 19, 22, 25, 28, 31, 34, 37]
    ...
    process.once("unhandledRejection", reason => console.log("Unhandled rejection: " + (reason && reason.stack), test.fail(1, "Unhandled rejection")));
    return mux.reduce((acc, item) => (acc.push(item.val), acc), []).then(arr => {
      ...
      test.end();
    }).catch(reason => console.log("Thrown: " + reason.stack, test.fail(1, "Unhandled rejection")));
  },

  test_map(test) {
    test.plan(2);
    const streams = [1, 3, 5].map(n => getStream(n, 3));
    return new MultiStream(streams).map(stream => stream.map(n => n.val + 1)).then(ms => {
  ...
  test_filter(test) {
    ...
    return new MultiStream(streams).filter(stream => stream.init % 2).then(ms => ms.map(stream => stream.map(n => n.val + 1))).then(ms => {
      test.ok(ms instanceof MultiStream, "Returns MultiStream (async)");
      return accumulate(ms.mux());
    }).then(acc => {
