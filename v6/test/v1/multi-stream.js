...
module.exports = {
  ...
  test_map(test) {
    ...
    test.doesNotThrow(() => {
      new MultiStream(streams).map(stream => stream.map(n => n.val + 1)).then(ms => {
  ...
  test_filter(test) {
    ...
    test.doesNotThrow(() => {
      new MultiStream(streams).filter(stream => stream.init % 2).then(ms => ms.map(stream => stream.map(n => n.val + 1))).then(ms => {
        test.ok(ms instanceof MultiStream, "Returns MultiStream (async)");
        return accumulate(ms.mux());
      }).then(acc => {
